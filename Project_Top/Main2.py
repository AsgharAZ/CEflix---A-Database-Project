...
class UI(QtWidgets.QMainWindow):
    ...
    def add_movie(self):
        ...
        # Generate Movie_ID manually or retrieve the next available ID (if required)
        ...
        cursor.execute(
            """
            INSERT INTO Movies (Movie_ID, Movie_Name, Summary, Year, Duration, Crew)
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (movie_id, movie_name, summary, year, duration, crew)
        )
        connection.commit()

        # Parse genres and insert into Genre table
        genre_list = [genre.strip() for genre in genre.split(",")]  # Split and trim whitespace
        for g in genre_list:
            cursor.execute(
                """
...
class Master(QtWidgets.QMainWindow):
    ...
    def populate_employee_table(self):
        """
        Populates the 'employeesTable' with employee data from the Project database.

        This function connects to the Project database, retrieves employee data,
        ...
        None
        """
        # TODO: Provide the  connection string to connect to the Project database
        connection = pyodbc.connect(
            "Driver={ODBC Driver 17 for SQL Server};"
            "Server=DESKTOP-VO1PBUT\\SQLSERVER;"
            "Database=Project;"
    ...
    def populate_customer_table(self):
        """
        Populates the 'customersTable' with customer data from the Project database.

        This function connects to the Project database, retrieves customer data,
    ...
    def populate_ship_table(self):
        """
        Populate the shippers table with data from the Project database.

        This function connects to the Project database, executes an SQL query to fetch
    ...
    def populate_product_table(self):
        """
        Populate the products table with data from the Project database.
    ...
    def insert_order(self):
        """
        Insert a new order into the Project database based on user input.
        ...
        Project database. After successfully inserting the order, it retrieves the
    ...
    def insert_order_details(self, order_id):
        ...
        Unit Price, Quantity, and Discount, into the 'Order Details' table in the Project
